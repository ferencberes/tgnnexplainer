...
class BaseExplainerTG(object):
    def __init__(
        ...
        self.tgnn_reward_wraper = TGNNRewardWraper(
            self.model, self.model_name, self.all_events, self.explanation_level
        )
        ...
        if self.model_name in ["tgat", "tgn"]:
            ...
            for i in range(num_layers):
                ...
                out_ngh_node_batch, out_ngh_eidx_batch, out_ngh_t_batch = (
                    ngh_finder.get_temporal_neighbor(
                        last_nodes,
                        last_ts,
    ...
    def _set_tgnn_wraper(self, event_idx):
        ...
        self.tgnn_reward_wraper.compute_original_score(
            self.base_events + self.candidate_events, event_idx
        )
