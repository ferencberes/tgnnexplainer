...
def _create_explainer_input(
    ...
    if model_name in ["tgat", "tgn"]:
        ...
        t_idx_u_emb = model.node_raw_embed[
            torch.tensor(event_idx_u, dtype=torch.int64, device=device), :
        ]
        t_idx_i_emb = model.node_raw_embed[
        ...
        t_idx_e_emb = model.edge_raw_embed[
            torch.tensor(
                [
                    event_idx_new,
                ],
        ...
        candidate_u_emb = model.node_raw_embed[
            torch.tensor(candidate_events_u, dtype=torch.int64, device=device), :
        ]
        candidate_i_emb = model.node_raw_embed[
        ...
        candidate_e_emb = model.edge_raw_embed[
            torch.tensor(candidate_events_new, dtype=torch.int64, device=device), :
        ]
...
class PGExplainerExt(BaseExplainerTG):
    ...
    def _init_explainer(self):
        self.explainer_model = self._create_explainer(
            self.model, self.model_name, self.device
        )
